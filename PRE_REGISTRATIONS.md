...
commit. The full pre-registration record (1,729 individual pre_reg_*.md files as of 2026-06-05)
lives in the THEOS2 private working archive. The entries below are the paper-cited
pre-registrations directly supporting the claims in the submitted Cryptologia manuscript;
the additional 1,700+ exploratory and bulletproofing pre-registrations are retained in the
working archive and available for adversarial verification under NDA. The nine key pre-reg
files referenced in `SUPPLEMENTARY_MATERIALS.md` (S5) are included in this repository under
`experiments/`.
...
| 2 | 2026-05 | **H1 — Language hypothesis.** The manuscript encodes a Mongolian-Tibetan hybrid pharmaceutical register, not a European language. Locked before R=14 was discovered. | Predates formal protocol (R=14 established at `f4a4b59`, post-dating H1); earliest verifiable H1 anchor: `c03ed14c` (batch158 context); see THEOS2 `git log --all --oneline \| grep -i H1` | CONFIRMED (Classical Mongolian three-case grammar; Tibetan pharmaceutical vocabulary; 19/23 syllabary characters mapped). | §2–§4 |
| 3 | 2026-05-22 | **Celestial cipher hypothesis.** §A uses a different rotation value (R=11) than the herbal section — predicted before the §A sweep ran. | `f4a4b59` | INITIAL FALSIFICATION SUPERSEDED. Initial §A confirmatory sweep classified this as falsified — R=14 appeared universal across all sections (p<0.001 against R=11). Subsequent pre-registered section-rotation refinement (paper §4, batches 5021–5031) re-confirmed §A=R=11 as the section-specific rotation, layered on the corpus-baseline R=14 wheel apparatus. Historical falsification record retained at NR-03 for transparency. | §4, §9, NR-03 |
...
| 5 | 2026-05-25 | **Syllabary v0.3 — e and p.** Predicted e→/e/ (pharmacological context marker) and p→/ph/ (aspirated bilabial) before the corpus confirmation analysis ran. | `f4037494` | CONFIRMED. Both characters confirmed at predicted values. Criterion 3 for e is conditional on f3r botanical ID (see RV-01). | §6 |
...
Original Result entries in VoynichPapers init commit `06ad82c` reported CONFIRMED for:
